...
<div class="">
	<div class="headBack">
		<el-row class="container">
				...
				<div class="headBox">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
						...
						<el-submenu index="/Share">
							<template slot="title"><i class="fa fa-wa fa-bookmark"></i> 分类</template>
            ...
            <el-menu-item index="/Introduce" ><i class="fa fa-wa fa-paper-plane"></i>关于我</el-menu-item>
						<el-autocomplete
						:readonly="readonly"
						autocomplete="off"
						v-model="inputState"
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/headbg06.jpeg)'}">
		...
		<div class="h-information">
                    ...
                    <img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/tou9.jpg'" alt="">

			<h2 class="h-description">

                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"海风依旧，事事皆欢"}}
...
import {getArticleIndex} from '../api/article.js'
import {
	Typeit
} from '../utils/plug.js'
export default {
	data() { //选项 / 数据
      ...
      readonly: true,
			...
			// path:'',//当前打开页面的路径
			input: '', //input输入内容
			headBg: 'url(static/img/headbg06.jpeg)', //头部背景图
			headTou: '', //头像
			projectList: '' ,//项目列表
      timeout: null,
      inputState: '',
    ...
    //输入数据queryString,返回建议数据通过cb返回数据
		querySearch(queryString, cb) {
      var allTitle= this.allTitle.map(item =>({name: item.id, value: item.title}));
